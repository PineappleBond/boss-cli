...
def login(qrcode: bool, cookie_source: str | None) -> None:
    ...
    def _maybe_refresh_browser_stoken(cred):
        try:
            from ..browser_login import BrowserLoginUnavailable, refresh_browser_credential
        except ImportError:
            return None

        try:
            return refresh_browser_credential(cred)
    ...
    def _finalize_login(cred, *, from_qr: bool = False) -> None:
        ...
        if (not from_qr) and message and "__zp_stoken__" in message:
            refreshed = _maybe_refresh_browser_stoken(cred)
            if refreshed:
                authenticated, message = verify_credential(refreshed, force_refresh=True)
                if authenticated:
                    console.print(f"[green]✅ 登录成功！[/green] ({len(refreshed.cookies)} cookies)")
                    return
