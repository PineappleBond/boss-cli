...
1. **Chrome Extension (Recommended)** — real-time cookie extraction from browser memory
2. **Saved cookies** — loads from `~/.config/boss-cli/credential.json`
3. **Browser cookies** — auto-detects installed browsers (Chrome, Firefox, Edge, Brave, Arc, Chromium, Opera, Vivaldi, Safari, LibreWolf)
4. **QR code login** — terminal QR output using Unicode half-blocks, scan with Boss 直聘 APP

### Chrome Extension (Real-time Cookie Extraction)

> **Why?** Boss 直聘's `__zp_stoken__` is generated by client-side JavaScript and not immediately written to disk by browsers. This causes frequent login failures: `环境异常 (__zp_stoken__ 已过期)`. The Chrome extension extracts cookies directly from browser memory, bypassing disk write delays.
...
1. **Load the extension**:
   ...
   - Click **Load unpacked**
   - Select the `chrome-extension/zhipin-cookie-extractor` folder

2. **Start the local service**:
   ```bash
   cd boss-cli
   python scripts/cookie_server.py
   ```
   This runs a local HTTP server on port 9876 to receive cookies from the extension.

3. **Login to Boss 直聘** in Chrome at `https://www.zhipin.com`

4. **Click the extension icon** to extract cookies

#### Features

| Feature | Description |
|---------|-------------|
| 🔄 Auto-refresh | Syncs cookies every 5 minutes (configurable: 1/3/5/10/30 min) |
| ⏱️ Last sync time | Shows when cookies were last extracted |
| 📋 Copy to clipboard | Export cookies as string for manual use |
| 💾 Export JSON | Save cookies to file |
| 🚀 Import to boss-cli | Send cookies to local service automatically |

#### Workflow

```
Chrome (logged in) → Extension extracts cookies → Local service → credential.json → boss-cli
```

The extension runs in background and auto-syncs, so you don't need to manually refresh cookies.
...
**Root cause**: `__zp_stoken__` is generated by client-side JavaScript and browsers don't write it to disk immediately while running.

**Solution (Recommended)**: Use the Chrome extension for real-time extraction:
1. Load extension from `chrome-extension/zhipin-cookie-extractor/` (developer mode)
2. Start local server: `python scripts/cookie_server.py`
3. Login to zhipin.com in Chrome
4. Click extension icon to sync cookies — auto-refreshes every 5 minutes

**Alternative**: Close Chrome completely, then run `boss login` to extract from disk.

**Q: `暂无投递记录` but I have applied**

Some features require fresh `__zp_stoken__`. Use the Chrome extension to get real-time cookies, or re-login from browser.
